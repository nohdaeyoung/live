#!/usr/bin/env python3
"""
👸 324 Planner — LangGraph 기반 PO 기획 에이전트

진짜 LangGraph StateGraph + compile + invoke 로 실행.
각 단계: 탐정요정(work) → 까칠한판사(critique) → PASS/REVISE 루프
...
/Users/openclaw-kong/.openclaw/workspace/324-os/planner/RESEARCHER.md 파일을 읽고 형식을 참고하라.
...
def node_work(state: PlannerState) -> dict:
    ...
    print(f"\n🧚 [{phase}/5] {PHASE_NAMES[phase]}{suffix} — 탐정요정 작업 중...")
...
def node_critique(state: PlannerState) -> dict:
    """까칠한판사가 현재 단계 검증"""
    phase = state["current_phase"]
    print(f"🧑‍⚖️  [{phase}/5] {PHASE_NAMES[phase]} — 까칠한판사 검증 중...")
...
def node_diagram_critique(state: PlannerState) -> dict:
    """까칠한판사가 다이어그램 vs 기획 정합성 검증"""
    print(f"🧑‍⚖️  다이어그램 정합성 검증 — 까칠한판사...")
...
def node_notion_review(state: PlannerState) -> dict:
    """까칠한판사가 노션 페이지를 읽고 포맷/정합성 검증"""
    print("🧑‍⚖️  노션 문서 품질 검증 — 까칠한판사...")
...
def node_notion_revise(state: PlannerState) -> dict:
    """탐정요정가 판사 피드백 기반으로 기획 내용 수정 → 카운트 증가"""
    rev = state.get("notion_revisions", 0)
    critique = state.get("notion_critique", "")
    print(f"\n🧚 노션 문서 수정 (수정 {rev + 1}차) — 탐정요정 작업 중...")

    # 판사 피드백에서 문제점 파악하고 탐정요정에게 수정 지시
    all_results = ""
    for i in range(1, 6):
        result = state["phase_results"].get(str(i), "")
        if result:
...
## 상황
노션에 업로드된 기획서가 까칠한판사 검증에서 REVISE 판정을 받았다.
...
""")
    ...
    if cmd == "run":
        ...
        print(f"""
╔══════════════════════════════════════════════════╗
║  👸 324 Planner — LangGraph Pipeline          ║
╚══════════════════════════════════════════════════╝
  Run ID: {run_id}
  아이디어: {idea[:60]}
  환경: {context[:60]}
...
""")
        ...
        hi_path = f"/tmp/324-planner-{run_id}-human-inputs.json"
