#!/usr/bin/env python3
"""
👸 324 Developer — Phase B: 구현 + 18단계 검수 파이프라인
...
# 탐정요정(scout)가 exec으로 코드 작성하는 단계
EXEC_STEPS = {5, 9, 13, 18}
# 까칠한판사(judge)가 문서 기반 검증하는 단계
REVIEW_STEPS = {2, 3, 4, 6, 7, 8, 10, 11, 12, 14, 15, 16, 17}
# 탐정요정가 계획/분석하는 단계 (exec 없이)
PLAN_STEPS = {1}
    ...
    review_focus = {
        ...
        6: ("목적 부합 검토",
            ...
            ["기능 완전성 — 설계서에 명시된 모든 페이지/뷰가 구현되었는가",
             ...
             "브랜딩 준수 — 설계서의 프로젝트명, 브랜드명, 컨셉 키워드가 코드에 반영되었는가 (예: '마스터 사미사', '산바람' 등)",
...
6. **브랜딩 불일치** → -3점
   ...
   - 예: 설계서 "마스터 사미사"인데 코드에 "AI Chat", "My App" 등 임의 이름 사용
...
Examples:
  ...
  python3 graph.py run "https://notion.so/..."              # 1번(계획)부터 풀 파이프라인""")
    ...
    if cmd == "run":
        ...
        print(f"""
╔══════════════════════════════════════════════════╗
║  🔨 324 Developer — Implementation Pipeline   ║
╚══════════════════════════════════════════════════╝
  Run ID: {run_id}
  설계서: {doc_url[:60]}
  기술: {tech[:60]}
