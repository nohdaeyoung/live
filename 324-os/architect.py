#!/usr/bin/env python3
"""
👸 324 Developer — Phase A: 개발 문서 생성 파이프라인
...
def node_work(state: DocState) -> dict:
    ...
    print(f"\n{icon} [{p}/7] {PHASE_NAMES[p]}{suffix} — 탐정요정...")
...
def node_critique(state: DocState) -> dict:
    p = state["current_phase"]
    print(f"🧑‍⚖️  [{p}/7] {PHASE_NAMES[p]} — 까칠한판사...")
...
def main():
  ...
  python3 doc_gen.py status RUN_ID""")
    ...
    if cmd == "run":
        ...
        print(f"""
╔══════════════════════════════════════════════════╗
║  🔧 324 Developer — Doc Gen Pipeline          ║
╚══════════════════════════════════════════════════╝
  Run ID: {run_id}
  기획서: {plan_url[:60]}
  기술: {tech[:60]}
