# 👸 비서공주 (Secretary Lobster) — 기능 명세서

> 최종 업데이트: 2026-02-15

## 개요

비서공주는 OpenClaw 기반의 AI 비서 에이전트로, 대표님(CEO)의 텔레그램 채널에서 운영됩니다.
...
> 코드: `324-os/logger.py`
> 테스트: `324-os/tests/test_logger.py` (174 unit tests)
...
### 인프라
- Firestore 프로젝트: `324-company-bip`
- 컬렉션: `chat_logs`
- 서비스 계정: `.firebase-service-account.json` (gitignored)
- 로그: `~/.openclaw/324-os-logger.log`
- PID: `~/.openclaw/324-os-logger.pid`
- 오프셋: `~/.openclaw/324-os-logger.offsets.json`
...
```
비서공주 → spawn(Researcher) → spawn(Planner) → 결과 announce
```
